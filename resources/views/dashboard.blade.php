<x-app-layout>
    ...
    <header
        ...
        <div>
            <h2
                ...
                Status Pengajuan</h2>
            <p class="text-body-md font-body-md text-on-surface-variant">Pantau progres layanan administratif Anda saat
                ini.</p>
        </div>
    </header>
    ...
    <div class="bg-primary-container/10 border-l-4 border-primary p-4 rounded-r-lg">
        <div class="flex gap-3">
            ...
            <p class="text-label-md text-primary font-medium">Kebijakan Layanan: Subdomain dan email di bawah domain
                murungrayakab.go.id hanya tersedia untuk Dinas/Unit Kerja di lingkungan Pemerintah Kabupaten Murung
                Raya. Layanan untuk tingkat Desa tidak tersedia melalui jalur ini.</p>
        </div>
    </div>
    ...
    <section class="flex flex-col gap-6">
        @forelse($applications as $app)
            <article
                ...
                <div
                    class="absolute left-0 top-0 bottom-0 w-1 {{ $app->status === 'selesai' || $app->status === 'tutup'
                        ? 'bg-success-emerald'
                        : ($app->status === 'rejected'
                            ? 'bg-error'
                            : ($app->status === 'proses'
                ...
                <div class="flex flex-col md:flex-row justify-between items-start gap-4">
                    <div>
                        <div class="flex items-center gap-2 mb-1">
                            ...
                            <span
                                class="bg-{{ $app->status === 'selesai' || $app->status === 'tutup'
                                    ? 'success-emerald'
                                    : ($app->status === 'rejected'
                                        ? 'error'
                                        : ($app->status === 'proses'
                                            ...
                                            : ($app->status === 'ditunda'
                                                ? 'orange-500'
                                                : 'blue-500'))) }}/10 text-{{ $app->status === 'selesai' || $app->status === 'tutup'
                                    ? 'success-emerald'
                                    : ($app->status === 'rejected'
                                        ? 'error'
                                        : ($app->status === 'proses'
                                            ...
                                            : ($app->status === 'ditunda'
                                                ? 'orange-600'
                                                : 'blue-600'))) }} text-label-sm font-label-sm px-2 py-0.5 rounded border border-{{ $app->status === 'selesai' || $app->status === 'tutup'
                                    ? 'success-emerald'
                                    : ($app->status === 'rejected'
                                        ? 'error'
                                        : ($app->status === 'proses'
                                            ...
                                            : ($app->status === 'ditunda'
                                                ? 'orange-500'
                                                : 'blue-500'))) }}/20 flex items-center gap-1">
                                <span class="material-symbols-outlined text-[14px]">
                                    {{ $app->status === 'selesai' || $app->status === 'tutup'
                                        ? 'check_circle'
                                        : ($app->status === 'rejected'
                                            ? 'cancel'
                                            : ($app->status === 'proses'
                        ...
                        <h3 class="text-headline-md font-headline-md text-primary">Pengajuan
                            {{ $app->type === 'subdomain' ? 'Subdomain' : 'Email Resmi' }}
                            {{ $app->unit_work ?? 'Dinas/Unit Kerja' }} @murungrayakab.go.id</h3>
                    ...
                    <div class="flex flex-col md:flex-row justify-between relative z-10 gap-6 md:gap-2">
                        ...
                        @foreach ($steps as $index => $step)
                            @php
                                $stepNum = $index + 1;
                                $isCompleted =
                                    ($currentStep > $stepNum && $app->status !== 'rejected') ||
                                    ($currentStep === $stepNum &&
                                        ($app->status === 'selesai' || $app->status === 'tutup'));
                                $isActive =
                                    $currentStep === $stepNum &&
                            ...
                            @endphp
                            <div
                                class="flex md:flex-col items-center md:items-center gap-3 md:gap-2 w-full md:w-1/5 relative">
                                <div
                                    class="w-8 h-8 rounded-full {{ $isRejected
                                        ? 'bg-error text-white'
                                        : ($isCompleted
                                            ? 'bg-success-emerald text-white'
                                            : ($isActive
                                                ? 'bg-status-pending text-on-secondary animate-pulse'
                                                : 'bg-surface text-on-surface-variant border-2 border-border-subtle')) }} flex items-center justify-center shrink-0 shadow-sm">
                                    @if ($isRejected)
                                ...
                                <div class="text-left md:text-center">
                                    <p
                                        class="text-label-sm font-label-sm {{ $isCompleted || $isActive ? 'text-primary font-bold' : 'text-on-surface-variant' }}">
                                        {{ $step['label'] }}
                                    </p>
                                    @if ($isRejected)
                                        <p class="text-caption font-caption text-error hidden md:block">Ditolak</p>
                                    @elseif($isCompleted)
                                        <p class="text-caption font-caption text-success-emerald hidden md:block">
                                            Selesai</p>
                                    @elseif($isActive)
                                        <p
                                            class="text-caption font-caption {{ $app->status === 'selesai' || $app->status === 'tutup' ? 'text-success-emerald' : 'text-status-pending' }} hidden md:block">
        ...
        @empty
            <div class="bg-surface border border-border-subtle rounded-xl p-12 text-center shadow-sm">
                ...
                <p class="text-body-md text-on-surface-variant mt-2">Anda belum mengajukan permohonan layanan apapun.
                </p>
            </div>
        @endforelse
    </section>
