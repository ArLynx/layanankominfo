<x-app-layout>
    <div class="flex-1 p-margin-mobile md:p-margin-desktop max-w-container-max mx-auto relative z-10 flex flex-col gap-8">
        <div class="mb-8">
            <h2 class="text-headline-md font-headline-md md:text-headline-lg md:font-headline-lg text-on-surface">Edit Pengajuan</h2>
            <p class="text-body-md font-body-md text-on-surface-variant mt-2">Perbaiki data pengajuan Anda yang sebelumnya ditolak agar dapat diproses kembali.</p>
        </div>
        ...
        @if ($errors->any())
            <div class="bg-error-container text-on-error-container p-4 rounded-lg border border-error flex items-start gap-3 animate-pulse">
                ...
                <div>
                    <p class="font-bold">Terjadi kesalahan saat pembaruan:</p>
        ...
        <div class="bg-surface border border-border-subtle rounded-xl shadow-sm overflow-hidden">
            <form method="POST" action="{{ route('requests.update', $application->id) }}" enctype="multipart/form-data" class="p-6 md:p-8" id="multi-step-form">
                @csrf
                @method('PATCH')
                ...
                <div class="block animate-fade-in" id="step-content-1">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                        ...
                        <label class="cursor-pointer">
                            <input class="peer sr-only service-type-input" name="type" type="radio" value="subdomain" {{ $application->type == 'subdomain' ? 'checked' : '' }} required>
                        ...
                        <!-- Email Pribadi -->
                        <label class="cursor-pointer">
                            <input class="peer sr-only service-type-input" name="type" type="radio" value="email_pribadi" {{ $application->type == 'email_pribadi' ? 'checked' : '' }}>
                            <div class="p-6 border-2 border-border-subtle rounded-xl peer-checked:border-primary peer-checked:bg-primary-fixed/20 hover:border-outline-variant transition-all h-full flex flex-col items-center text-center">
                                <div class="w-16 h-16 rounded-full bg-surface-container-high flex items-center justify-center mb-4 text-primary">
                        ...
                        <!-- Email Satker -->
                        <label class="cursor-pointer">
                            <input class="peer sr-only service-type-input" name="type" type="radio" value="email_satker" {{ $application->type == 'email_satker' ? 'checked' : '' }}>
                            <div class="p-6 border-2 border-border-subtle rounded-xl peer-checked:border-primary peer-checked:bg-primary-fixed/20 hover:border-outline-variant transition-all h-full flex flex-col items-center text-center">
                                <div class="w-16 h-16 rounded-full bg-surface-container-high flex items-center justify-center mb-4 text-primary">
                ...
                <div class="hidden animate-fade-in" id="step-content-2">
                    ...
                    <div class="space-y-5">
                        <div>
                            ...
                            <input class="w-full px-4 py-2 bg-surface border border-border-subtle rounded-lg focus:outline-none focus:ring-2 focus:ring-primary focus:border-primary transition-shadow text-body-md" name="details[nama_pemohon]" value="{{ old('details.nama_pemohon', $application->details['nama_pemohon'] ?? '') }}" placeholder="Masukkan nama lengkap" type="text">
                        </div>
                        <div>
                            <label class="block text-label-md font-label-md text-on-surface mb-1">Alasan Pengajuan <span class="text-error">*</span></label>
                            <textarea class="w-full px-4 py-2 bg-surface border border-border-subtle rounded-lg focus:outline-none focus:ring-2 focus:ring-primary focus:border-primary transition-shadow text-body-md min-h-[100px]" name="reason" placeholder="Jelaskan alasan kebutuhan layanan ini secara detail (min 10 karakter)" required>{{ old('reason', $application->reason) }}</textarea>
                ...
                <div class="hidden animate-fade-in" id="step-content-3">
                    ...
                    <div id="form-subdomain" class="hidden space-y-6">
                        <div class="flex flex-col gap-2">
                            ...
                            <div class="flex">
                                <input class="flex-grow px-4 py-3 bg-surface border border-outline-variant rounded-l-lg text-body-md focus:outline-none focus:ring-2 focus:ring-primary focus:border-primary transition-all" name="details[proposed_subdomain]" value="{{ old('details.proposed_subdomain', $application->details['proposed_subdomain'] ?? '') }}" placeholder="misal: dpmptsp" required type="text">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div class="flex flex-col gap-2">
                                ...
                                <input class="w-full px-4 py-3 bg-surface border border-outline-variant rounded-lg text-body-md focus:outline-none focus:ring-2 focus:ring-primary focus:border-primary transition-all" name="details[pic_name]" value="{{ old('details.pic_name', $application->details['pic_name'] ?? '') }}" required type="text">
                            </div>
                            <div class="flex flex-col gap-2">
                                <label class="text-label-md font-label-md text-on-surface">Kontak PJ (WhatsApp) <span class="text-error">*</span></label>
                                <input class="w-full px-4 py-3 bg-surface border border-outline-variant rounded-lg text-body-md focus:outline-none focus:ring-2 focus:ring-primary focus:border-primary transition-all" name="details[pic_contact]" value="{{ old('details.pic_contact', $application->details['pic_contact'] ?? '') }}" required type="text">
                            </div>
                        </div>
                    </div>
                    ...
                    <div id="form-email_pribadi" class="hidden space-y-6">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div class="flex flex-col gap-2">
                                ...
                                <input class="w-full px-4 py-3 bg-surface border border-outline-variant rounded-lg text-body-md focus:outline-none focus:ring-2 focus:ring-primary focus:border-primary transition-all" name="details[nama]" value="{{ old('details.nama', $application->details['nama'] ?? '') }}" required type="text">
                            </div>
                            <div class="flex flex-col gap-2">
                                <label class="text-label-md font-label-md text-on-surface">NIP <span class="text-error">*</span></label>
                                <input class="w-full px-4 py-3 bg-surface border border-outline-variant rounded-lg text-body-md focus:outline-none focus:ring-2 focus:ring-primary focus:border-primary transition-all" name="details[nip]" value="{{ old('details.nip', $application->details['nip'] ?? '') }}" required type="text">
                            </div>
                            <div class="flex flex-col gap-2">
                                <label class="text-label-md font-label-md text-on-surface">Jabatan <span class="text-error">*</span></label>
                                <input class="w-full px-4 py-3 bg-surface border border-outline-variant rounded-lg text-body-md focus:outline-none focus:ring-2 focus:ring-primary focus:border-primary transition-all" name="details[jabatan]" value="{{ old('details.jabatan', $application->details['jabatan'] ?? '') }}" required type="text">
                            </div>
                            <div class="flex flex-col gap-2">
                                <label class="text-label-md font-label-md text-on-surface">Instansi <span class="text-error">*</span></label>
                                <input class="w-full px-4 py-3 bg-surface border border-outline-variant rounded-lg text-body-md focus:outline-none focus:ring-2 focus:ring-primary focus:border-primary transition-all" name="details[instansi]" value="{{ old('details.instansi', $application->details['instansi'] ?? '') }}" required type="text">
                            </div>
                            <div class="flex flex-col gap-2">
                                <label class="text-label-md font-label-md text-on-surface">Email Pribadi <span class="text-error">*</span></label>
                                <input class="w-full px-4 py-3 bg-surface border border-outline-variant rounded-lg text-body-md focus:outline-none focus:ring-2 focus:ring-primary focus:border-primary transition-all" name="details[email]" value="{{ old('details.email', $application->details['email'] ?? '') }}" required type="email">
                            </div>
                            <div class="flex flex-col gap-2">
                                <label class="text-label-md font-label-md text-on-surface">No. HP WA Aktif <span class="text-error">*</span></label>
                                <input class="w-full px-4 py-3 bg-surface border border-outline-variant rounded-lg text-body-md focus:outline-none focus:ring-2 focus:ring-primary focus:border-primary transition-all" name="details[no_hp_wa]" value="{{ old('details.no_hp_wa', $application->details['no_hp_wa'] ?? '') }}" required type="text">
                            </div>
                        </div>
                        <div class="flex flex-col gap-2">
                            <label class="text-label-md font-label-md text-on-surface">Nama Akun yang Diinginkan <span class="text-error">*</span></label>
                            <div class="flex">
                                <input class="flex-grow px-4 py-3 bg-surface border border-outline-variant rounded-l-lg text-body-md focus:outline-none focus:ring-2 focus:ring-primary focus:border-primary transition-all" name="details[nama_akun]" value="{{ old('details.nama_akun', $application->details['nama_akun'] ?? '') }}" placeholder="nama.pegawai" required type="text">
                                <span class="px-4 py-3 bg-surface-container border border-l-0 border-outline-variant rounded-r-lg text-label-md text-on-surface-variant">@murungrayakab.go.id</span>
                            </div>
                        </div>
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div class="flex flex-col gap-2">
                                ...
                                <select class="w-full px-4 py-3 bg-surface border border-outline-variant rounded-lg text-body-md focus:outline-none focus:ring-2 focus:ring-primary focus:border-primary transition-all" name="details[jenis_layanan]" required>
                                    <option value="baru" {{ old('details.jenis_layanan', $application->details['jenis_layanan'] ?? '') == 'baru' ? 'selected' : '' }}>Permohonan Baru</option>
                                    <option value="reset" {{ old('details.jenis_layanan', $application->details['jenis_layanan'] ?? '') == 'reset' ? 'selected' : '' }}>Reset Password</option>
                                    <option value="hapus" {{ old('details.jenis_layanan', $application->details['jenis_layanan'] ?? '') == 'hapus' ? 'selected' : '' }}>Hapus Akun</option>
                                    <option value="ganti" {{ old('details.jenis_layanan', $application->details['jenis_layanan'] ?? '') == 'ganti' ? 'selected' : '' }}>Ganti Nama Akun</option>
                                </select>
                            </div>
                            <div class="flex flex-col gap-2">
                                ...
                                <select class="w-full px-4 py-3 bg-surface border border-outline-variant rounded-lg text-body-md focus:outline-none focus:ring-2 focus:ring-primary focus:border-primary transition-all" name="details[pengajuan_untuk]" required>
                                    <option value="diri_sendiri" {{ old('details.pengajuan_untuk', $application->details['pengajuan_untuk'] ?? '') == 'diri_sendiri' ? 'selected' : '' }}>Diri Sendiri</option>
                                    <option value="orang_lain" {{ old('details.pengajuan_untuk', $application->details['pengajuan_untuk'] ?? '') == 'orang_lain' ? 'selected' : '' }}>Orang Lain</option>
                                </select>
                            </div>
                        </div>
                    </div>
                    ...
                    <div id="form-email_satker" class="hidden space-y-6">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div class="flex flex-col gap-2">
                                ...
                                <input class="w-full px-4 py-3 bg-surface border border-outline-variant rounded-lg text-body-md focus:outline-none focus:ring-2 focus:ring-primary focus:border-primary transition-all" name="details[nama_instansi]" value="{{ old('details.nama_instansi', $application->details['nama_instansi'] ?? '') }}" required type="text">
                            </div>
                            <div class="flex flex-col gap-2">
                                <label class="text-label-md font-label-md text-on-surface">Nama Akun Dinas <span class="text-error">*</span></label>
                                <div class="flex">
                                    <input class="flex-grow px-4 py-3 bg-surface border border-outline-variant rounded-l-lg text-body-md focus:outline-none focus:ring-2 focus:ring-primary focus:border-primary transition-all" name="details[nama_akun]" value="{{ old('details.nama_akun', $application->details['nama_akun'] ?? '') }}" placeholder="nama.satker" required type="text">
                                    <span class="px-4 py-3 bg-surface-container border border-l-0 border-outline-variant rounded-r-lg text-label-md text-on-surface-variant">@murungrayakab.go.id</span>
                                </div>
                            </div>
                            <div class="flex flex-col gap-2">
                                <label class="text-label-md font-label-md text-on-surface">Nama Penanggung Jawab <span class="text-error">*</span></label>
                                <input class="w-full px-4 py-3 bg-surface border border-outline-variant rounded-lg text-body-md focus:outline-none focus:ring-2 focus:ring-primary focus:border-primary transition-all" name="details[nama_pj]" value="{{ old('details.nama_pj', $application->details['nama_pj'] ?? '') }}" required type="text">
                            </div>
                            <div class="flex flex-col gap-2">
                            ...
                            <div class="flex flex-col gap-2">
                                ...
                                <input class="w-full px-4 py-3 bg-surface border border-outline-variant rounded-lg text-body-md focus:outline-none focus:ring-2 focus:ring-primary focus:border-primary transition-all" name="details[email_pribadi]" value="{{ old('details.email_pribadi', $application->details['email_pribadi'] ?? '') }}" required type="email">
                            </div>
                            <div class="flex flex-col gap-2">
                            ...
                            <div class="flex flex-col gap-2">
                                ...
                                <select class="w-full px-4 py-3 bg-surface border border-outline-variant rounded-lg text-body-md focus:outline-none focus:ring-2 focus:ring-primary focus:border-primary transition-all" name="details[jenis_layanan]" required>
                                    <option value="reset" {{ old('details.jenis_layanan', $application->details['jenis_layanan'] ?? '') == 'reset' ? 'selected' : '' }}>Reset Password</option>
                                    <option value="ganti" {{ old('details.jenis_layanan', $application->details['jenis_layanan'] ?? '') == 'ganti' ? 'selected' : '' }}>Ganti Nama Akun</option>
                                    <option value="pj" {{ old('details.jenis_layanan', $application->details['jenis_layanan'] ?? '') == 'pj' ? 'selected' : '' }}>Pergantian Penanggung Jawab</option>
                                </select>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="mt-10 flex flex-col gap-2">
                        ...
                        @if($application->document_path)
                            <p class="text-xs text-success-emerald mt-2 flex items-center gap-1">
        ...
        <div class="mt-8 pt-6 border-t border-border-subtle flex justify-between items-center">
            <button class="px-6 py-2 rounded-lg text-primary text-label-md font-label-md hover:bg-surface-container transition-colors" id="btn-prev" type="button">
                Kembali
            </button>
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            const updateUI = () => {
                ...
                if (currentStep === totalSteps) {
                    btnNext.textContent = 'Perbarui Pengajuan';
                } else {
            ...
            btnNext.addEventListener('click', () => {
                if (currentStep < totalSteps) {
                    ...
                    const typeSelected = document.querySelector('input[name="type"]:checked');
                    const activeFormId = typeSelected.value === 'subdomain' ? 'form-subdomain' : 
                                      (typeSelected.value === 'email_pribadi' ? 'form-email_pribadi' : 'form-email_satker');
