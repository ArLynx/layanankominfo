<x-pimpinan-layout title="Persetujuan Email Pribadi">
    <header class="flex items-start md:items-center justify-between border-b border-border-subtle pb-6">
        <div>
            ...
            <p class="text-body-md font-body-md text-on-surface-variant">Verifikasi dokumen pengajuan email pribadi</p>
        </div>
    </header>
                    ...
                    @if($emailPribadi->status === 'tunda')
                        <div>
                            ...
                            <textarea name="catatan_pimpinan" form="approval-form" rows="4"
                                      ...
                                      placeholder="Masukkan catatan pimpinan...">{{ old('catatan_pimpinan', $emailPribadi->catatan_pimpinan) }}</textarea>
                            @error('catatan_pimpinan')
                                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                            @enderror
                        </div>
                    @elseif($emailPribadi->catatan_pimpinan)
                        <div>
                            <label class="text-sm text-gray-500">Catatan Pimpinan</label>
        ...
        function openApprovalModal(action) {
            ...
            if (action === 'approve') {
                ...
                confirmBtn.onclick = function() {
                    form.action = "{{ route('pimpinan.email-pribadi.approve', $emailPribadi) }}";
                    form.submit();
                };
            } else {
                ...
                confirmBtn.onclick = function() {
                    form.action = "{{ route('pimpinan.email-pribadi.reject', $emailPribadi) }}";
                    form.submit();
                };
            }
        }
