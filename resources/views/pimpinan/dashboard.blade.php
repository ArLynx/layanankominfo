    ...
    <div class="flex-1 p-margin-mobile md:p-margin-desktop max-w-container-max mx-auto relative z-10 flex flex-col gap-8">
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-7 gap-4">
            ...
            <div class="bg-surface border border-border-subtle rounded-xl p-4 shadow-sm border-l-4 border-l-yellow-500">
                <p class="text-caption font-caption text-on-surface-variant mb-1">Proses</p>
                <p class="text-headline-md font-headline-md text-yellow-600">{{ $stats['proses'] }}</p>
            </div>
            <div class="bg-surface border border-border-subtle rounded-xl p-4 shadow-sm border-l-4 border-l-orange-500">
                <p class="text-caption font-caption text-on-surface-variant mb-1">Ditunda</p>
                <p class="text-headline-md font-headline-md text-orange-600">{{ $stats['ditunda'] }}</p>
            </div>
            ...
            <div class="bg-surface border border-border-subtle rounded-xl p-4 shadow-sm border-l-4 border-l-gray-500">
                <p class="text-caption font-caption text-on-surface-variant mb-1">Tutup</p>
                <p class="text-headline-md font-headline-md text-gray-600">{{ $stats['tutup'] }}</p>
            </div>
            <div class="bg-surface border border-border-subtle rounded-xl p-4 shadow-sm border-l-4 border-l-red-500">
                ...
                <p class="text-headline-md font-headline-md text-red-600">{{ $stats['rejected'] }}</p>
            </div>
        </div>
        ...
        <div class="bg-surface border border-border-subtle rounded-xl shadow-sm overflow-hidden">
            <div class="p-6 border-b border-border-subtle bg-surface-gray">
                <h3 class="text-headline-md font-headline-md text-on-surface">Daftar Seluruh Permohonan</h3>
            </div>
            <div class="overflow-x-auto">
                <table class="w-full text-left border-collapse">
                    <thead>
                        <tr class="border-b border-border-subtle bg-surface-gray/50">
                            <th class="px-6 py-4 text-label-sm font-label-sm text-on-surface-variant">ID</th>
                            <th class="px-6 py-4 text-label-sm font-label-sm text-on-surface-variant">Pemohon</th>
                            <th class="px-6 py-4 text-label-sm font-label-sm text-on-surface-variant">Layanan</th>
                    ...
                    <tbody class="text-body-md font-body-md text-on-surface">
                        @forelse($applications as $app)
                            <tr class="border-b border-border-subtle hover:bg-surface-gray/30 transition-colors">
                                <td class="px-6 py-4 font-mono text-sm">APP-{{ str_pad($app->id, 4, '0', STR_PAD_LEFT) }}-{{ date('Y') }}</td>
                                <td class="px-6 py-4">
                                    <div class="font-medium">{{ $app->user->name }}</div>
                                    <div class="text-caption text-on-surface-variant">{{ $app->user->email }}</div>
                                </td>
                                <td class="px-6 py-4">
                                ...
                                <td class="px-6 py-4">
                                    <span class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md text-label-sm font-label-sm 
                                        {{ $app->status === 'terbuka' ? 'bg-blue-100 text-blue-700 border border-blue-200' : '' }}
                                        {{ $app->status === 'proses' ? 'bg-yellow-100 text-yellow-700 border border-yellow-200' : '' }}
                                        {{ $app->status === 'ditunda' ? 'bg-orange-100 text-orange-700 border border-orange-200' : '' }}
                                        {{ $app->status === 'selesai' ? 'bg-green-100 text-green-700 border border-green-200' : '' }}
                                        {{ $app->status === 'tutup' ? 'bg-gray-100 text-gray-700 border border-gray-200' : '' }}
            ...
            <div class="p-4 border-t border-border-subtle bg-surface-gray">
                {{ $applications->links() }}
            </div>
        </div>
    </div>
